...
- **结果页增强**：支持黑名单过滤（含正则别名）、商品屏蔽/恢复、筛选条件本地持久化与筛选后即时刷新。
- **AI 调用稳定性**：支持流式响应解析与 `reasoning_content` 兼容回退；当网关拒绝 `response_format` 时自动降级；修复 `NO_PROXY` 含 IPv6 CIDR 导致的 httpx 崩溃。
- **任务执行稳态**：命中 AI 分析中止阈值后立即停止重试，降低无效请求与风控风险。
- **运行时一致性**：统一任务/提示词/账号状态在不同运行路径（Web/API/爬虫）下的加载行为，减少 SQLite/PG 双路径偏差。
- **设置与安全**：设置页联动 AI 流式开关与失败阈值；渠道测试校验收敛到目标渠道；AI 测试报错信息脱敏。
- **前端可用性**：区域选择器支持“无子级区域”直接选择（如广东省）；修复 i18n 对 webhook/cron 示例文本的误解析。
- **通知配置修复**：通知配置优先读取 `.env`，避免历史运行态值污染。
- **工程化补充**：新增 PostgreSQL 迁移脚本与运行时依赖整理，便于 SQLite → PG 演进。
